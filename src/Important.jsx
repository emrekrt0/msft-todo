...
const supabase = createClient(
  ...
  export default function Important() {
    ...
    supabase
    .channel('room1')
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'todo' }, handleInserts)
    .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'todo' }, handleInserts)
    .subscribe()
                ...
                <div key={task.id} className="baseAdd addTask box-shadow mb-20">
                    ...
                    <ul>
                        <li className="baseAddInput-important">
                            <div className="whatTodo">{task.todo} </div> {task.important ? <div className="importantCheck"><button onClick={changeImportant}>⭐</button></div> : null}
                        </li>
                    </ul>
                </div>
            ))}
